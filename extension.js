...
function activate(context) {
	let disposable = vscode.commands.registerCommand('fzf', async function () {
		...
		const terminal = vscode.window.createTerminal({
			...
			name: 'fzf',
			shellPath: 'zsh',
			shellArgs: ['-c', `pwd > ${tempPath} && rg --line-number --no-heading --color=always -g '!yarn.lock' '' | fzf --ansi --print0 --delimiter : --preview 'batcat --style=numbers --color=always --highlight-line {2} {1}' --preview-window +{2}-/2 >> ${tempPath}`],
		});
		...
		const listener = vscode.window.onDidChangeActiveTerminal(async () => {
			if (terminal.exitStatus !== undefined) {
				...
				editor.selection =  new vscode.Selection(lineNumber - 1, column, lineNumber - 1, column);
